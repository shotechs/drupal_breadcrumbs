...
class BreadcrumbBuilder implements BreadcrumbBuilderInterface {
  ...
  public function __construct(RequestContext $context, AccessManagerInterface $access_manager, RequestMatcherInterface $router, InboundPathProcessorInterface $path_processor, ConfigFactoryInterface $config_factory, TitleResolverInterface $title_resolver, AccountInterface $current_user, CurrentPathStack $current_path, MenuLinkManager $menu_link_manager) {
    ...
    $this->config = $config_factory->get('breadcrumb.settings');
  ...
  public function build(RouteMatchInterface $route_match) {
    ...
    if (!$this->config->get(BreadcrumbConstants::INCLUDE_TITLE_SEGMENT)) {
    ...
    if (isset($path_elements[0])) {
      ...
      if (!($this->config->get(BreadcrumbConstants::LANGUAGE_PATH_PREFIX_AS_SEGMENT))) {
    ...
    while (count($path_elements) > 0) {
      ...
      if ($this->config->get(BreadcrumbConstants::EXCLUDED_PATHS)) {
        $config_textarea = $this->config->get(BreadcrumbConstants::EXCLUDED_PATHS);
      ...
      if ($route_request) {
        ...
        if ($access->isAllowed()) {
          if ($this->config->get(BreadcrumbConstants::TITLE_FROM_PAGE_WHEN_AVAILABLE)) {
            $title = $this->titleResolver->getTitle($route_request, $route_match->getRouteObject());
          }
          if (!isset($title)) {

            if ($this->config->get(BreadcrumbConstants::USE_MENU_TITLE_AS_FALLBACK)) {
          ...
          if ($i == 0
              && $this->config->get(BreadcrumbConstants::INCLUDE_TITLE_SEGMENT)
              && !$this->config->get(BreadcrumbConstants::TITLE_SEGMENT_AS_LINK)) {
      ...
      elseif ($this->config->get(BreadcrumbConstants::INCLUDE_INVALID_PATHS)) {
    ...
    if ($this->config->get(BreadcrumbConstants::INCLUDE_HOME_SEGMENT)) {
      if ($path && '/' . $path != $front && $path != $curr_lang) {
        $links[] = Link::createFromRoute($this->config->get(BreadcrumbConstants::HOME_SEGMENT_TITLE), '<front>');
      }
    }
    $links = array_reverse($links);

    if ($this->config->get(BreadcrumbConstants::REMOVE_REPEATED_SEGMENTS)) {
